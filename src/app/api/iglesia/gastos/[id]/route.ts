...
/** GET /api/iglesia/gastos/[id] — devuelve el gasto para prefilear el form */
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await ctx.supabase
      .from("gastos")
      .select("id, filial_id, categoria_gasto_id, fecha, monto, descripcion, forma_pago, numero_factura")
      .eq("id", id)
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Mantener el campo legacy `categoria` (texto) sincronizado
    ...
    const { data, error } = await ctx.supabase
      .from("gastos")
      .update({
        filial_id, categoria_gasto_id, fecha, monto,
