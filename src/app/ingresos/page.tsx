...
export default function IngresosPage() {
  ...
  async function eliminarConfirmado() {
    ...
    const res = await fetchWithSupabaseSession(`/api/iglesia/ingresos/${confirmDel.id}`, { method: "DELETE" });
    if (res.ok) setIngresos((prev) => prev.filter((x) => x.id !== confirmDel.id));
    setConfirmDel(null);
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-lg font-semibold tracking-tight text-slate-900">Ingresos</h1>
          <p className="mt-0.5 text-xs text-slate-500">Diezmos, ofrendas y votos por filial</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <ReferenciaBoton tipo="ingresos" />
            ...
            <table className="w-full text-sm">
              ...
              <tfoot>
                <tr className="border-t-2 border-slate-300 bg-slate-50 font-semibold">
                  <td colSpan={8} className="px-4 py-2.5 text-right">TOTAL</td>
      ...
      {confirmDel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={() => setConfirmDel(null)}>
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-[#4FAEB2]/20" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-slate-900">¿Eliminar este ingreso?</h3>
      ...
      {nuevoMov && (
        <MovimientoModal defaultTipo="ingreso" onClose={() => setNuevoMov(false)} onSaved={cargar} />
      )}
    </div>
  );
}
